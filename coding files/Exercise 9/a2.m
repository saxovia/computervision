...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  y1 = logistic(z1);
  z2 = w2 * y1;
  normalizer = log_sum_exp_over_rows(z2);
  log_class_prob = z2 - repmat(normalizer, size(z2,1), 1);
  class_prob = exp(log_class_prob);
  ret.hid_to_class = (class_prob - data.targets) * y1' / size(data.inputs, 2);
  term1 = (class_prob - data.targets)' * w2;
  d_loss_by_d_z1 = zeros(size(y1));
  for i = 1:size(term1,1)
        row = term1(i,:) .* (y1(:,i) .* (1 - y1(:,i)))';
        d_loss_by_d_z1(:,i) = row';
  end
  ret.input_to_hid = d_loss_by_d_z1 * data.inputs' / size(data.inputs, 2);
